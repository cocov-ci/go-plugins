...
func newCocovIssue(rootPath string, i Issue, kind cocov.IssueKind) (*common.CocovIssue, bool) {
	if kind == issueKindException {
		return newExceptionIssue(rootPath, i)
	}

	filePath := filepath.Join(rootPath, i.Pos.Filename)
	ccIssue := common.NewCocovIssue(kind, i.Pos.Line, i.Pos.Line, filePath, i.Text)
	return ccIssue, true
}

func newExceptionIssue(rootPath string, i Issue) (*common.CocovIssue, bool) {
	...
	text := splitMessage[1]
	filePath := filepath.Join(rootPath, i.Pos.Filename)
	ccIssue := common.NewCocovIssue(kind, i.Pos.Line, i.Pos.Line, filePath, text)
